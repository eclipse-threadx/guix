...
#include "multi_themes_16bpp_resources.h"
#include "multi_themes_16bpp_specifications.h"
...
extern GX_STUDIO_DISPLAY_INFO multi_themes_16bpp_display_table[];
...
GX_CHAR binres_pathname[] = "multi_themes_16bpp_resources.bin";
...
UINT simple_window_event_handler(GX_WINDOW *widget, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_THEME_1, GX_EVENT_RADIO_SELECT):
        gx_display_theme_install(multi_themes_16bpp_display_table[MAIN_DISPLAY].display, main_display_theme_table[MAIN_DISPLAY_THEME_1]);
        break;
    ...
    case GX_SIGNAL(ID_THEME_3, GX_EVENT_RADIO_SELECT):
        gx_display_theme_install(multi_themes_16bpp_display_table[MAIN_DISPLAY].display, main_display_theme_table[MAIN_DISPLAY_THEME_3]);
        break;
...
UINT load_binary_resource_data_to_ram()
{
    ...
    memcpy(pathname + len, Japanese_pathname, sizeof(Japanese_pathname));
    p_file = fopen(pathname, "rb");
...
GX_THEME *new_theme = GX_NULL;
    ...
    if (status == GX_SUCCESS)
    {
        ...
        gx_display_theme_install(multi_themes_16bpp_display_table[MAIN_DISPLAY].display, new_theme);
