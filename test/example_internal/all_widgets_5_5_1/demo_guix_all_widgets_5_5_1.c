...
extern GX_STUDIO_DISPLAY_INFO all_widgets_5_5_1_display_table[];
...
VOID start_guix(void)
{
    ...
    gx_canvas_create(&composite_canvas, GX_NULL,
        all_widgets_5_5_1_display_table[0].display,
        GX_CANVAS_COMPOSITE,
    ...
    gx_window_root_create(&nested_root, "nested_root", &animation_canvas, GX_STYLE_BORDER_NONE, ID_ANIMATION_CANVAS_ROOT_WINDOW, &NestedWindow.gx_widget_size);
...
UINT text_screen_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_SL_INPUT, GX_EVENT_TEXT_EDITED):
        gx_single_line_text_input_buffer_get(&text_screen.text_screen_text_input_1, &text_buffer, &string_size, &buffer_size);
        if ((string_size == text_screen.text_screen_text_input_1.gx_single_line_text_input_string_size) &&
            (buffer_size == text_screen.text_screen_text_input_1.gx_single_line_text_input_buffer_size))
        {
            gx_multi_line_text_view_text_set(&text_screen.text_screen_text_view_1, text_buffer);
        }
        break;

    case GX_SIGNAL(ID_ML_INPUT, GX_EVENT_TEXT_EDITED):
        gx_multi_line_text_input_buffer_get(&text_screen.text_screen_text_input_2, &text_buffer, &string_size, &buffer_size);
        if ((string_size == text_screen.text_screen_text_input_2.gx_multi_line_text_view_string_byte_count) &&
...
UINT radial_slider_screen_event_process(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_RADIAL_SLIDER, GX_EVENT_SLIDER_VALUE):
        gx_numeric_prompt_value_set(&radial_slider_screen.radial_slider_screen_radial_slider_value, event_ptr->gx_event_payload.gx_event_intdata[0]);
        break;
...
GX_CHAR temp[10];
    ...
    strcpy_s(entry->text, sizeof(entry->text), " List Entry #");
    gx_utility_ltoa(index + 1, temp, 10);
    strcat_s(entry->text, sizeof(entry->text), temp);
