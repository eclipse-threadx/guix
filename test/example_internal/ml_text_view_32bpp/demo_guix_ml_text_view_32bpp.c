...
#include <stdlib.h>
#include "gx_api.h"
#include "ml_text_view_32bpp_resources.h"
#include "ml_text_view_32bpp_specifications.h"
...
void PopulateMLTexts(GX_CHAR *pathname)
{
    ...
    if (pFile)
    {
        ...
        read_size = fread(buffer, 1, file_size, pFile);

        buffer[read_size] = '\0';
        str.gx_string_ptr = buffer;
        str.gx_string_length = read_size;
        gx_multi_line_text_view_text_set_ext(&pMainScreen->window_text_view, &str);
        fclose(pFile);
    }
}
