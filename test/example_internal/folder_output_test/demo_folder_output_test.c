...
extern GX_STUDIO_DISPLAY_INFO folder_output_test_display_table[];
...
/* comment out the macro define RUN_MAIN_DISPLAY when you want run sencond display. */
#define RUN_MAIN_DISPLAY

#ifdef RUN_MAIN_DISPLAY
    gx_studio_display_configure(MAIN_DISPLAY, win32_graphics_driver_setup_24xrgb,
        MAIN_DISPLAY_LANGUAGE_JAPANESE, MAIN_DISPLAY_THEME_1, &root);

    /* create the button screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_button_screen", (GX_WIDGET *) root, (GX_WIDGET **) &pButtonScreen);

    /* create the text widget screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_text_screen", GX_NULL, (GX_WIDGET **) &pTextScreen);

    /* create the windows screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_window_screen", GX_NULL, (GX_WIDGET **) &pWindowScreen);
    PopulateDropList();

    /* create the indicator screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_indicator_screen", GX_NULL, (GX_WIDGET **) &pIndicatorScreen);

    /* create the popup screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_popup_modal_derived_derived", GX_NULL, (GX_WIDGET **) &pPopupScreen);

    /* create the popup screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_base_derived_1", GX_NULL, GX_NULL);

    /* create the popup screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_popup_modal_derived_1", GX_NULL, GX_NULL);

    /* create the popup screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_popup_modal_derived_derived_1", GX_NULL, GX_NULL);

    /* create circular gauge screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_gauge_screen", GX_NULL, (GX_WIDGET **)&pGaugeScreen);

    /* create sprite screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_sprite_screen", GX_NULL, (GX_WIDGET **)&pSpriteScreen);

    /* create menu screen */
    gx_studio_named_widget_create("MAIN_DISPLAY_menu_screen", GX_NULL, GX_NULL);

    gx_studio_named_widget_create("MAIN_DISPLAY_rotate_screen", GX_NULL, GX_NULL);

    gx_studio_named_widget_create("MAIN_DISPLAY_child_template_screen", GX_NULL, GX_NULL);
    ...
    gx_canvas_create(&composite_canvas, GX_NULL,
        folder_output_test_display_table[0].display,
        GX_CANVAS_COMPOSITE,
    ...
    gx_widget_fill_color_set(&NestedWindow, GX_COLOR_ID_MAIN_DISPLAY_BLUE, GX_COLOR_ID_MAIN_DISPLAY_BLUE, GX_COLOR_ID_MAIN_DISPLAY_BLUE);
...
#else
    gx_studio_display_configure(SECONDARY, win32_graphics_driver_setup_24xrgb,
        SECONDARY_LANGUAGE_CHINESE, SECONDARY_THEME_1, &root);

    /* create the button screen */
    gx_studio_named_widget_create("SECONDARY_button_screen", (GX_WIDGET *)root, (GX_WIDGET **)&pButtonScreen);

    /* create the text widget screen */
    gx_studio_named_widget_create("SECONDARY_text_screen", GX_NULL, (GX_WIDGET **)&pTextScreen);

    /* create the windows screen */
    gx_studio_named_widget_create("SECONDARY_window_screen", GX_NULL, (GX_WIDGET **)&pWindowScreen);

    /* create the indicator screen */
    gx_studio_named_widget_create("SECONDARY_indicator_screen", GX_NULL, (GX_WIDGET **)&pIndicatorScreen);

    /* create the popup screen */
    gx_studio_named_widget_create("SECONDARY_popup_modal_derived_derived", GX_NULL, (GX_WIDGET **)&pPopupScreen);

    /* create circular gauge screen */
    gx_studio_named_widget_create("SECONDARY_gauge_screen", GX_NULL, (GX_WIDGET **)&pGaugeScreen);

    /* create sprite screen */
    gx_studio_named_widget_create("SECONDARY_sprite_screen", GX_NULL, (GX_WIDGET **)&pSpriteScreen);

    /* create menu screen */
    gx_studio_named_widget_create("SECONDARY_menu_screen", GX_NULL, GX_NULL);

    gx_studio_named_widget_create("SECONDARY_rotate_screen", GX_NULL, GX_NULL);
    gx_widget_show(root);
#endif
...
UINT buttons_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_BUTTON, GX_EVENT_CLICKED):
        button_click_count++;
        gx_numeric_prompt_value_set(&MAIN_DISPLAY_button_screen.MAIN_DISPLAY_button_screen_count_prompt, button_click_count);
        break;

    case GX_SIGNAL(ID_CHECKBOX, GX_EVENT_TOGGLE_ON):
        gx_widget_style_remove(&MAIN_DISPLAY_button_screen.MAIN_DISPLAY_button_screen_pixelmap_button1, GX_STYLE_ENABLED);
        break;

    case GX_SIGNAL(ID_CHECKBOX, GX_EVENT_TOGGLE_OFF):
        gx_widget_style_add(&MAIN_DISPLAY_button_screen.MAIN_DISPLAY_button_screen_pixelmap_button1, GX_STYLE_ENABLED);
        break;
...
UINT text_screen_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_SL_INPUT, GX_EVENT_TEXT_EDITED):
        gx_single_line_text_input_buffer_get(&MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_text_input_1, &text_buffer, &string_size, &buffer_size);
        if ((string_size == MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_text_input_1.gx_single_line_text_input_string_size) &&
            (buffer_size == MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_text_input_1.gx_single_line_text_input_buffer_size))
        {
            gx_multi_line_text_view_text_set(&MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_text_view_1, text_buffer);
        }
        break;

    case GX_SIGNAL(ID_ML_INPUT, GX_EVENT_TEXT_EDITED):
        gx_multi_line_text_input_buffer_get(&MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_text_input_2, &text_buffer, &string_size, &buffer_size);
        if ((string_size == MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_text_input_2.gx_multi_line_text_view_text.gx_string_length) &&
            (buffer_size == MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_text_input_2.gx_multi_line_text_input_buffer_size))
        {
            gx_multi_line_text_view_text_set(&MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_text_view_1, text_buffer);
        }
        break;
    ...
    case GX_EVENT_TIMER:
        if (event_ptr->gx_event_payload.gx_event_timer_id == NUMBER_ANIMATION_TIMER)
        {
            ...
            gx_numeric_prompt_value_set(&MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_numeric_prompt, numeric_prompt_value);
            gx_numeric_pixelmap_prompt_value_set(&MAIN_DISPLAY_text_screen.MAIN_DISPLAY_text_screen_numeric_pixelmap_prompt, numeric_prompt_value);
        }
        break;
...
VOID PopulateDropList(void)
{
    ...
    MAIN_DISPLAY_WINDOW_SCREEN_CONTROL_BLOCK *control = (MAIN_DISPLAY_WINDOW_SCREEN_CONTROL_BLOCK *) pWindowScreen;
    GX_DROP_LIST *drop = &control->MAIN_DISPLAY_window_screen_drop_list;
...
GX_CONST GX_CHAR *text;
    ...
    gx_system_string_get(GX_STRING_ID_MAIN_DISPLAY_STRING_44, &text);
