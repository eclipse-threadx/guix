...
#include "screenshot_to_bitmap_monochrome_specifications.h"
...
GX_CHAR bmp_file_aligned[] = "test_screenshot_aligned.bmp";
GX_CHAR golden_bmp_file_aligned[] = "screenshot_aligned.bmp";
...
GX_CHAR bmp_file_aligned[] = "screenshot_aligned.bmp";
...
VOID start_guix(void)
{
    ...
    gx_studio_display_configure(PRIMARY, win32_graphics_driver_setup_monochrome,
...
VOID generate_screen_shot()
{
    ...
    gx_utility_canvas_to_bmp(root->gx_window_root_canvas, &scroll_wheel_screen.gx_widget_size, write_data_callback);
    /* Close file */
...
{
    int len = string_length_get(pathname, MAX_STRING_LENGTH) - 1;
...
{
    GX_RECTANGLE size;
    GX_CHAR pathname[MAX_STRING_LENGTH];
    INT len;
    ...
    size.gx_rectangle_right = 606;
    memcpy(pathname + len, bmp_file_aligned, sizeof(bmp_file_aligned));
    fp = fopen(pathname, "wb");
    if (!fp)
    {
        printf("Cannot open file: %s\n", pathname);
