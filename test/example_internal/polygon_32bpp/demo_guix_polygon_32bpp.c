...
#include "polygon_32bpp_resources.h"
#include "polygon_32bpp_specifications.h"
...
{
GX_PROMPT *pp;
static GX_CHAR width[10];

    gx_widget_find(pMainWin, ID_LINE_WIDTH_DISPLAY, 0, &pp);

    if(pp)
    {
        gx_utility_ltoa(line_width, width, 10);
        gx_prompt_text_set(pp, width);
    }
}
