...
#include "synergy_32bpp_16bppdriver_specifications.h"
...
UINT main_event_handler(GX_WINDOW *window, GX_EVENT *myevent)
{
    ...
    {
        case GX_SIGNAL(ID_COMPRESSED, GX_EVENT_RADIO_SELECT):
        compressed = GX_TRUE;
        gx_system_dirty_mark(pic_window);
        break;

        case GX_SIGNAL(ID_UNCOMPRESSED, GX_EVENT_RADIO_SELECT):
        compressed = GX_FALSE;
        gx_system_dirty_mark(pic_window);
        break;
