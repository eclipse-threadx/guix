...
CUSTOM_CHECKBOX_INFO alarm_checkbox_info = {
    SWITCH_BTTON_ID,
    GX_PIXELMAP_ID_SWITCH_BG_BIG,
    GX_PIXELMAP_ID_SWITCH_ACTIVE_BIG,
    GX_PIXELMAP_ID_SWITCH_DISACTIVE_BIG,
    4,
    23
};
...
VOID alarm_list_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
    ...
    if (!result)
    {
        gx_utility_rectangle_define(&childsize, 0, 0, 240, 70);
        ...
        gx_utility_rectangle_define(&childsize, 10, 10, 68, 43);
        custom_checkbox_create(&row->checkbox, &row->widget, &alarm_checkbox_info, &childsize);

        /* Create hour prompt. */
        gx_utility_rectangle_define(&childsize, 110, 3, 155, 43);
        ...
        gx_prompt_font_set(&row->time_hour, GX_FONT_ID_MIDDLE_NUMBER);

        /* Create time break prompt. */
        gx_icon_create(&row->dot_up, NULL, &row->widget, GX_PIXELMAP_ID_DOT, GX_STYLE_TRANSPARENT, 0, 158, 12);
        gx_icon_create(&row->dot_down, NULL, &row->widget, GX_PIXELMAP_ID_DOT, GX_STYLE_TRANSPARENT, 0, 158, 27);
        ...
        /* Create minute prompt. */
        gx_utility_rectangle_define(&childsize, 165, 3, 235, 43);
        ...
        gx_prompt_font_set(&row->time_minute, GX_FONT_ID_MIDDLE_NUMBER);

        /* Create am/pm prompt. */
        gx_utility_rectangle_define(&childsize, 140, 41, 210, 60);
