...
#define SCRATCHPAD_PIXELS  640 * 480 * 2
...
UINT root_window_event_handler(GX_WINDOW *window, GX_EVENT *event_ptr)
{
    ...
    case GX_EVENT_KEY_DOWN:
        if (event_ptr->gx_event_payload.gx_event_ushortdata[0] == 0x1b)
        {
            /* When ESC key down, switch to main screen. */
...
#else
    /* Insert your real time clock management here. */
#endif
}
