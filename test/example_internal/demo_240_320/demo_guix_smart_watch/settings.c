...
CUSTOM_CHECKBOX_INFO settings_checkbox_info = {
    SWITCH_BTTON_ID,
...
VOID setting_list_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
    ...
    if (!result)
    {
        gx_utility_rectangle_define(&childsize, 0, 0, 240, 50);
        ...
        gx_utility_rectangle_define(&childsize, 15, 7, 65, 35);
        settings_checkbox_info.widget_id = current_list_info[index].id;
        custom_checkbox_create(&row->checkbox, &row->widget, &settings_checkbox_info, &childsize);

        gx_utility_rectangle_define(&childsize, 70, 0, 220, 40);
        ...
        gx_utility_rectangle_define(&childsize, 215, 7, 240, 38);
...
VOID setting_list_children_create(GX_VERTICAL_LIST *list)
{
    ...
    gx_utility_rectangle_define(&size, 0, 60, 240, 280);
