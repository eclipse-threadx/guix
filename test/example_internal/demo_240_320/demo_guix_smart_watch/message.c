...
#define MSG_VIEW_HEIGHT       30

#define MAX_MSG_INFO_COUNT     6
#define MAX_MSG_COUNT          5
#define TEXT_BOX_WIDTH       180
...
VOID msg_search_window_animation_update()
{
    ...
    if (((animation_shift < 0) && (size.gx_rectangle_left <= 10)) ||
        ((animation_shift > 0) && (size.gx_rectangle_left >= 165)))
    {
...
VOID msg_list_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
    ...
    if (!result)
    {
        ...
        gx_utility_rectangle_define(&size, 125, 3, 205, 30);
        ...
        gx_utility_rectangle_define(&size, 3, MSG_TITLE_HEIGHT + 8, 210, MSG_TITLE_HEIGHT + 8 + MSG_VIEW_HEIGHT);
        ...
        gx_multi_line_text_view_font_set(&row->content, GX_FONT_ID_SMALL);
...
VOID dialog_row_create(GX_WIDGET *parent, GX_WIDGET *widget, INT index, MSG_DATA *data, INT top)
{
    ...
    {
        gx_utility_rectangle_shift(&size, 30, 0);
