...
BEGIN_MESSAGE_MAP(express_dialog, CDialog)
 ...
 ON_WM_CLOSE()
 ON_WM_SETTINGCHANGE()
 ON_MESSAGE(WM_DPICHANGED, &express_dialog::OnDpiChanged)
END_MESSAGE_MAP()
...
//    lpCreateStruct->dwExStyle |= WS_EX_APPWINDOW;
    ...
    int offset = MulDiv((TITLE_BAR_HEIGHT - CLOSE_BUTTON_HEIGHT - 3) >> 1, m_dpi, DEFAULT_DPI_96);
...
INT_PTR express_dialog::DoModal()
{
    ...
    int point_size = GetDialogTemplateFontPointSize(DEFAULT_DLG_FONT_POINT_SIZE, m_dpi, m_text_scaler);
...
BOOL CALLBACK EnumChildProc(HWND hwnd, LPARAM lParam)
{
    ...
    default:
        child->SetFont(param->font);
        rect.left = MulDiv(rect.left, param->new_scale, param->old_scale);
        rect.right = MulDiv(rect.right, param->new_scale, param->old_scale);
        rect.top = MulDiv(rect.top, param->new_scale, param->old_scale);
        rect.bottom = MulDiv(rect.bottom, param->new_scale, param->old_scale);
        child->MoveWindow(&rect);
        break;
    }
...
///////////////////////////////////////////////////////////////////////////////
void express_dialog::CreateDialogFont(int text_scaler, int dpi)
{
    ...
    int point_size = GetDialogTemplateFontPointSize(DEFAULT_DLG_FONT_POINT_SIZE, dpi, text_scaler);
...
///////////////////////////////////////////////////////////////////////////////
void express_dialog::ChangeDialogFontSize(int dpi, BOOL resize_window)
{
    ...
    if (new_dpi <= 0)
    {
        new_dpi = GetDpiForStudioWindow(GetSafeHwnd());
    }

    if ((new_text_scaler != m_text_scaler) || (new_dpi != m_dpi))
    {
        int old_scale = m_dpi * m_text_scaler;
        int new_scale = new_dpi * new_text_scaler;

        if (old_scale <= 0)
        {
            old_scale = DEFAULT_DPI_96 * DEFAULT_TEXT_SCALER;
        }

        CreateDialogFont(new_text_scaler, new_dpi);
        ...
        if (resize_window)
        {
            ...
            int width = MulDiv(rect.Width(), new_scale, old_scale);
            int height = MulDiv(rect.Height(), new_scale, old_scale);
            rect.right = rect.left + width;
            rect.bottom = rect.top + height;
            MoveWindow(&rect);
        }

        FontResizeParam param = { &m_dialog_font, old_scale, new_scale };
...
///////////////////////////////////////////////////////////////////////////////
LRESULT express_dialog::OnDpiChanged(WPARAM wParam, LPARAM lParam)
{
    ...
    ChangeDialogFontSize(LOWORD(wParam), FALSE);
    return 0;
}
