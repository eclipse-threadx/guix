#ifndef _WINDOW_SERVICE_PROVIDER_
#define _WINDOW_SERVICE_PROVIDER_

class screen_generator; // forward reference

class window_service_provider : public widget_service_provider
{
    public:
        window_service_provider();

        virtual int GetType() {return GX_TYPE_WINDOW;}
        virtual CString GetTypeName(widget_info *) {return CString("GX_TYPE_WINDOW");}
        virtual CString GetShortName() {return CString("window");}
        virtual CString GetVarDeclaration();
        virtual CString GetControlBlockName() {return CString("GX_WINDOW");}
