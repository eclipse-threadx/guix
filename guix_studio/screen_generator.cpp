...
CString out("");
    ...
    /* Write the prototypes of the static screen flow helpers. Declaring them up
       front keeps the generated file compliant with projects that require a
       visible prototype for every function definition.  */
    ...
    /* gx_studio_action_parent_find() and gx_studio_animation_execute() are only
       reachable from the GX_ACTION_TYPE_ANIMATION handler, and they reference the
       animation API which is compiled out when the animation pool is empty. Guard
       both so that a screen flow project builds with GX_ANIMATION_POOL_SIZE = 0.  */
    out = "#if (GX_ANIMATION_POOL_SIZE > 0)\n"
          "static GX_WIDGET *gx_studio_action_parent_find(GX_WIDGET *current, GX_CONST GX_STUDIO_ACTION *action)\n"
          "{\n"
          "GX_WIDGET *parent = GX_NULL;\n"
          "GX_STUDIO_WIDGET *widget_define;\n\n"
