...
properties_win::properties_win()
{
    ...
    mpProject = NULL;
    m_dpi = 0;
    m_item_space = PROPERTY_ITEM_SPACE;
...
void properties_win::SetControlSize(int dpi)
{
    if (dpi <= 0)
    {
        dpi = m_dpi;
    ...
    m_item_space = GetScaledValue(PROPERTY_ITEM_SPACE, dpi, text_scaler);
    ...
    if (dc)
    {
        ...
        m_item_height = max(m_item_height, tm.tmHeight + (m_item_space << 1));
    }

    m_row_height = m_item_height + m_item_space;
    m_leftcol_width = GetScaledValue(LEFT_COLUMN_WIDTH, dpi, text_scaler);
}

///////////////////////////////////////////////////////////////////////////////
void properties_win::UpdateDpiResources(int dpi)
{
    SetControlSize(dpi);
    ResourcesChanged();
    Invalidate();
...
void properties_win::AddPushButton(char *description, int id)
{
    ...
    size.left += m_item_space;
    size.right -= m_item_space;
    size.bottom = size.top + m_item_height + (m_item_space << 1);

    CButton *pButton = new CButton();
    pButton->Create(cs, WS_CHILD | WS_TABSTOP | BS_PUSHBUTTON, size, this, id);

    m_top_pos += m_row_height + (m_item_space << 1);
}
