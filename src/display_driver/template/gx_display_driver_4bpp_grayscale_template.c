...
*/
/**************************************************************************/
static void generic_buffer_toggle_4bpp_grayscale(GX_CANVAS *canvas, GX_RECTANGLE *copy)
{
    ...
    copy_aligned.gx_rectangle_left &= 0xfffe;
    copy_aligned.gx_rectangle_right |= 0x0001;
    ...
    copy_width /= 2;            // copying UBYTEs
    pGetRow = (ULONG *) canvas -> gx_canvas_memory;
    pGetRow += copy_aligned.gx_rectangle_top * (canvas->gx_canvas_x_resolution / 2);
    pGetRow += copy_aligned.gx_rectangle_left / 2;
    ...
    pPutRow += (canvas ->gx_canvas_display_offset_y + copy_aligned.gx_rectangle_top) * DISPLAY_XRES / 2;
    pPutRow += (canvas ->gx_canvas_display_offset_x + copy_aligned.gx_rectangle_left) / 2;
    ...
    for (row = 0; row < copy_height; row++)
    {
        ...
        pGetRow += canvas->gx_canvas_x_resolution / 1;
        pPutRow += DISPLAY_XRES / 1;
    }
    HardwareCacheFlush(flushaddress, copy_height * DISPLAY_XRES / 2);
}


/**************************************************************************/
/*
  ConfigureDisplayHardware4bpp_grayscale
...
*/
/**************************************************************************/
static void ConfigureDisplayHardware4bpp_grayscale(GX_DISPLAY *display)
{
    INT buffer_size;
    ...
    buffer_size = DISPLAY_XRES * DISPLAY_YRES / 2;
...
/**************************************************************************/
UINT template_graphics_driver_setup_4bpp_grayscale(GX_DISPLAY *display)
{
    /* install all of the generic palette mode drawing functions */
    _gx_display_driver_4bpp_grayscale_setup(display, GX_NULL, generic_buffer_toggle_4bpp_grayscale);
    
    display -> gx_display_handle = DISPLAY_HANDLE;
    ConfigureDisplayHardware4bpp_grayscale(display);
