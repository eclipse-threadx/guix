...
$content = Replace-Required $content "(?m)^#define GUIX_HOTFIX_VERSION\s+'.*'" "#define GUIX_HOTFIX_VERSION $($release.HotfixDefine)" 'GUIX_HOTFIX_VERSION'
...
if ($release.Hotfix) {
    Write-Host "Hotfix letter: $($release.Hotfix)"
}
