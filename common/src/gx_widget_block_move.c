...
GX_VALUE     height;
    ...
    {
        status = _gx_canvas_drawing_initiate(canvas, widget, block);
        ...
        if (status == GX_SUCCESS)
        {
            if (_gx_canvas_block_move(block, (GX_VALUE)x_shift, (GX_VALUE)y_shift, &dirty) == GX_SUCCESS)
            {
                ...
                UINT inner_status = _gx_canvas_drawing_initiate(canvas, widget, &dirty);

                if (inner_status == GX_SUCCESS)
                {
                    widget->gx_widget_draw_function(widget);
                    _gx_canvas_drawing_complete(canvas, GX_FALSE);
                }
                else if (inner_status == GX_NO_VIEWS)
                {
                    _gx_canvas_drawing_complete(canvas, GX_FALSE);
                }
