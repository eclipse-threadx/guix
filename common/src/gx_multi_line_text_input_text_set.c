...
UINT glyph_len;
    ...
    while (string.gx_string_length > 0)
    {
        ...
        {
            /* string now points at the byte after ch. Read it through string, so
               that its length bounds the access: ch[1] would rely on the caller
               having NUL terminated a GX_STRING, which carries its own length
               and need not be terminated at all. */
