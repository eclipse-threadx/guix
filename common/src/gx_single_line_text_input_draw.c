...
GX_RECTANGLE          saved_dirty;
    ...
    _gx_utility_rectangle_overlap_detect(&saved_dirty, &client, &overlap);
    status = _gx_canvas_drawing_initiate(canvas, widget, &overlap);
    ...
    if (status == GX_DRAW_NESTING_EXCEEDED)
    {
        ...
        context -> gx_draw_context_dirty = overlap;
    }
