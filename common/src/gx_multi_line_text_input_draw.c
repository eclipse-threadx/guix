...
GX_RECTANGLE          saved_dirty;
    ...
    else
    {
        ...
        if ((input -> gx_multi_line_text_view_text.gx_string_length) && (font))
        {
            ...
            /* Remember the current context and its clipping rectangle, in case the
               draw context stack turns out to be full.  */
            context = _gx_system_current_draw_context;
            saved_dirty = context -> gx_draw_context_dirty;

            _gx_utility_rectangle_overlap_detect(&saved_dirty, &client, &draw_area);
            status = _gx_canvas_drawing_initiate(canvas, (GX_WIDGET *)input, &draw_area);

            if (status == GX_DRAW_NESTING_EXCEEDED)
            {
                /* The context stack is full, so no context was pushed. The context
                   that would have been pushed is created for this same widget, so
                   it differs from the current one only in its clipping rectangle.
                   Narrow the current context's clipping rectangle instead and draw
                   through it, rather than dropping the text. The rectangle is
                   restored once the text is drawn.  */
            ...
            if ((status == GX_SUCCESS) || (status == GX_DRAW_NESTING_EXCEEDED))
            {
                ...
                {
                    /* Nothing was pushed, so _gx_canvas_drawing_complete() must
                       not be called: it would pop the caller's context. Restore
                       the clipping rectangle borrowed above instead.  */
            ...
            {
                /* Any other status, such as GX_INVALID_MEMORY_SIZE when drawing
                   into a partial frame buffer, is also returned before a context
                   is pushed, so there is nothing to draw and nothing to
                   complete.  */
            }
        }
    }
