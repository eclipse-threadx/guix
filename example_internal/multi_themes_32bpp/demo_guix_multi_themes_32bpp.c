...
#include "multi_themes_32bpp_resources.h"
#include "multi_themes_32bpp_specifications.h"
...
UINT load_binres_theme(INT theme_id);

extern GX_STUDIO_DISPLAY_INFO multi_themes_32bpp_display_table[];

GX_CONST GX_THEME *main_display_theme_table[1] = {GX_NULL};
GX_STRING *main_display_language_table[MAIN_DISPLAY_LANGUAGE_TABLE_SIZE];

GX_CONST GX_STRING **language_table = GX_NULL;


GX_UBYTE *binres_root_address = GX_NULL;
GX_CHAR binres_pathname[] = "multi_themes_32bpp_resources.bin";
...
VOID tx_application_define(void *first_unused_memory)
{
    ...
    if (load_binary_resource_data_to_ram() == GX_SUCCESS)
    {
        /* Load theme 1. */
        load_binres_theme(MAIN_DISPLAY_THEME_1);

        /* Load language table. */
        gx_binres_language_table_load_ext(binres_root_address, &language_table);

        for (index = 0; index < MAIN_DISPLAY_LANGUAGE_TABLE_SIZE; index++)
        {
            main_display_language_table[index] = language_table[index];
        }
    }
...
UINT simple_window_event_handler(GX_WINDOW *widget, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_THEME_2, GX_EVENT_RADIO_SELECT):
        load_binres_theme(MAIN_DISPLAY_THEME_2);
        break;

    case GX_SIGNAL(ID_THEME_3, GX_EVENT_RADIO_SELECT):
        load_binres_theme(MAIN_DISPLAY_THEME_3);
        break;

    case GX_SIGNAL(ID_THEME_4, GX_EVENT_RADIO_SELECT):
        load_binres_theme(MAIN_DISPLAY_THEME_4);
        break;

    case GX_SIGNAL(ID_THEME_5, GX_EVENT_RADIO_SELECT):
        load_binres_theme(MAIN_DISPLAY_THEME_5);
        break;
...
UINT  status = GX_SUCCESS;
    ...
    if (main_display_theme_table[0])
    {
        memory_free(main_display_theme_table[0]);
        main_display_theme_table[0] = GX_NULL;
    }

    status = gx_binres_theme_load(binres_root_address, theme_id, &main_display_theme_table[0]);

    if (main_display_theme_table[0])
    {
        gx_display_theme_install(multi_themes_32bpp_display_table[MAIN_DISPLAY].display, main_display_theme_table[0]);
    }
    return status;
}
