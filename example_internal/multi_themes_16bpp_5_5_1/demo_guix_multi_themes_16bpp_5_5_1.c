...
#include "multi_themes_16bpp_5_5_1_specifications.h"
...
UINT load_binres_language_table(GX_UBYTE *root_address, INT active_language);

extern GX_STUDIO_DISPLAY_INFO multi_themes_16bpp_5_5_1_display_table[];
...
GX_CHAR binres_pathname[] = "..//..//multi_themes_16bpp_5_5_1_resources.bin";
...
UINT simple_window_event_handler(GX_WINDOW *widget, GX_EVENT *event_ptr)
{
    ...
    case GX_SIGNAL(ID_THEME_1, GX_EVENT_RADIO_SELECT):
        gx_display_theme_install(multi_themes_16bpp_5_5_1_display_table[MAIN_DISPLAY].display, main_display_theme_table[MAIN_DISPLAY_THEME_1]);
        break;

    case GX_SIGNAL(ID_THEME_2, GX_EVENT_RADIO_SELECT):
        gx_display_theme_install(multi_themes_16bpp_5_5_1_display_table[MAIN_DISPLAY].display, main_display_theme_table[MAIN_DISPLAY_THEME_2]);
        break;

    case GX_SIGNAL(ID_THEME_3, GX_EVENT_RADIO_SELECT):
        gx_display_theme_install(multi_themes_16bpp_5_5_1_display_table[MAIN_DISPLAY].display, main_display_theme_table[MAIN_DISPLAY_THEME_3]);
        break;
    ...
    case GX_SIGNAL(ID_ENGLISH, GX_EVENT_RADIO_SELECT):
        gx_system_language_table_set((GX_CHAR ***)&main_display_language_table, MAIN_DISPLAY_LANGUAGE_TABLE_SIZE, MAIN_DISPLAY_STRING_TABLE_SIZE);
    ...
    case GX_SIGNAL(ID_JAPANESE, GX_EVENT_RADIO_SELECT):
        load_binres_language_table(binres_root_address, 0);
        break;

    case GX_SIGNAL(ID_SPANISH, GX_EVENT_RADIO_SELECT):
        load_binres_language_table(binres_root_address, 1);
        break;
    }
...
UINT load_binary_resource_data_to_ram()
{
    ...
    fopen_s(&p_file, binres_pathname, "rb");
...
UINT  status = GX_SUCCESS;
    ...
    if (theme)
    {
        gx_display_theme_install(multi_themes_16bpp_5_5_1_display_table[MAIN_DISPLAY].display, theme);
    }
...
UINT  status = GX_SUCCESS;
    ...
    if (language_table)
    {
        gx_system_language_table_set((GX_CHAR ***)language_table, 2, MAIN_DISPLAY_STRING_TABLE_SIZE);
        gx_system_active_language_set(active_language);
    }
    return status;
}
