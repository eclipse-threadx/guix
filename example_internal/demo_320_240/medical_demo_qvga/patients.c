...
#define MR_NUM_SHIFT                            160
#define ROOM_NUM_SHIFT                          78
#define VERTICAL_LIST_BOTTOM                    198
#define PATIENT_LIST_VISIBLE_ROWS               6
...
void patient_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
    ...
    if (!result)
    {
        ...
        gx_utility_rectangle_define(&childsize, 10, 3, 200, 27);
    ...
    gx_prompt_font_set(&row->name, GX_FONT_ID_SMALL);
    gx_prompt_font_set(&row->mr_num, GX_FONT_ID_SMALL);
    gx_prompt_font_set(&row->room_num, GX_FONT_ID_SMALL);
...
GX_WIDGET *widget = (GX_WIDGET *) window;
    ...
    case GX_SIGNAL(ID_PATIENT_LIST, GX_EVENT_LIST_SELECT):
        ...
        gx_vertical_list_selected_index_get(list, &index);
        strcpy(current_patient_name, patient_list[index].name);
        break;
    }
