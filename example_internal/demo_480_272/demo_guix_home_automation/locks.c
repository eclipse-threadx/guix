...
{
    GX_RESOURCE_ID enable_disable_text_id;

    if (lock_info.lock_enabled)
    {
        enable_disable_text_id = GX_STRING_ID_ENABLED;
    }
    else
    {
        enable_disable_text_id = GX_STRING_ID_DISABLED;
    }

    gx_prompt_text_id_set(&locks_page_1.locks_page_1_enable_disable_text, enable_disable_text_id);
}
...
static VOID update_lock_unlock_button_appearance()
{
    ...
    if (lock_info.lock_enabled)
    {
        ...
        {
            string_id = GX_STRING_ID_UNLOCK_ALL;
        ...
        {
            string_id = GX_STRING_ID_LOCK_ALL;
    ...
    /* Set child button text. */
    gx_multi_line_text_button_text_id_set(&locks_page_1.locks_page_1_lock_unlock_text, string_id);

    /* Set child prompt text color. */
    gx_text_button_text_color_set((GX_TEXT_BUTTON *)&locks_page_1.locks_page_1_lock_unlock_text, color_id, color_id, color_id);
