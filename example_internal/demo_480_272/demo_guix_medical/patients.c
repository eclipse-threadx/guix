...
#define MR_NUM_SHIFT                            158
#define ROOM_NUM_SHIFT                          100
#define ADMIT_DATE_SHIFT                        100
#define PATIENT_LIST_VISIBLE_ROWS               5
...
void patient_row_create(GX_VERTICAL_LIST *list, GX_WIDGET *widget, INT index)
{
    ...
    if (!result)
    {
        gx_utility_rectangle_define(&childsize, list->gx_window_client.gx_rectangle_left + 10, 0,
                                                list->gx_window_client.gx_rectangle_right - 10, 22);
